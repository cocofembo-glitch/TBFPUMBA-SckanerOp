...
LANG = {
    "uk": {
        "app_name": "🔥 TBFPUMBA-SckanerOp v7.0 🔥",
...
if __name__ == "__main__":
    ...
    os.system('clear')
    rainbow_print(LANG[lang]['app_name'], lang)
    rainbow_print(LANG[lang]['app_by'], lang)
    rainbow_print("========================================", lang)
    
    loading_bar(LANG[lang]['loading'], lang, 0.02)
    
    # Перевірка аргументу командного рядка
    if len(sys.argv) > 1:
        target_path = sys.argv[1]
        if os.path.exists(target_path):
            rainbow_print(f"\n📂 Обрано шлях з аргументу: {target_path}", lang)
        else:
            print(f"{RED}{LANG[lang]['invalid_path']}{RESET}")
    ...
    rainbow_print(f"\n{LANG[lang]['selected']}{target_path}", lang)
    
    threats = []
    while True:
        answer = input(f"\n{LANG[lang]['scan_confirm']}").lower()
        if answer in ['y', 'yes']:
            break
        elif answer in ['n', 'no']:
            rainbow_print(LANG[lang]['scan_cancel'], lang)
            sys.exit()
        elif answer == 'help':
            show_help(lang)
            continue
            ...
            print(f"❗ {LANG[lang]['cmd_not_found']}")
    ...
    if results:
        ...
        print("\n" + "="*50)
        rainbow_print(LANG[lang]['scan_result_title'], lang)
        print("="*50)
        
        if critical:
            color_print(LANG[lang]['critical_title'], RED)
            for file, level, msg in critical:
                color_print(f"  🔴 {file} -> {msg}", RED)
        
        if suspicious:
            color_print(LANG[lang]['suspicious_title'], YELLOW)
            for file, level, msg in suspicious:
                color_print(f"  🟡 {file} -> {msg}", YELLOW)
        
        color_print(f"\n{LANG[lang]['found'].format(len(results))}", BLUE)
        color_print(f"💡 {LANG[lang]['delete_hint']}", CYAN)
        threats = results
    else:
        rainbow_print(f"\n{LANG[lang]['clean']}", lang)
        threats = []
    
    print(f"\n{LANG[lang]['scan_done'].format(duration)}")
    ...
    while True:
        ...
        if cmd == '-TBF':
            ...
            rainbow_print(LANG[lang]['exit_msg'], lang)
            sys.exit()
        else:
            print(f"❌ {LANG[lang]['unknown_cmd']}")
